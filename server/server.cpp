...
// Constructor
Server::Server(int port) : hosting_port(port), addrlen(sizeof(address)) {
    // 初始化工作者狀態為閒置
...
void* Server::handle_client(void* arg) {
    ...
    while (true) {
        ssize_t message_len = read(client_socket, buffer, BUFFER_SIZE - 1);
        if (message_len <= 0) {
        ...
        if (clean_message == "Exit") {
            ...
            send(client_socket, return_message.c_str(), return_message.length(), 0);
            break;
        }
